...
public class List {
   ...
   public List (List d) { // конструктов для копирования списка
    ...
    public void apened(String str) {
        List a = new List (str);// переделали введенную строку в список
        List vs = new List(a); // создали копию
        Node h = searchEndBlock();
        h.next=vs.head; // смена указателей
    }

    public void apened(char a){
